...
void compute_prog1(int want_display,int want_nf) 
{
...

  {
	...
	options._env._nb=2; /* If we want to compute a normalForm : 
	...
	for(;i<nb;i+=1+want_nf)
	  {
	    ...
	    /* Import the internal representation of each polynomial computed by FGb */
	    {
	      ...
	      int code=FGB(export_poly_INT_gmp2)(nb_vars,nb_mons,cfs,Mons,output_basis[i]);
